...
class AuxiliaryAI:
    ...
    async def update_topic_and_summary(self, node, recursive=False):
        ...
        이 노드와 자식 노드들의 내용을 바탕으로, 기존 내용과 키워드를 유지하면서 최소한으로 요약하라. 요약은 50자 이내로 간결하게 생성하라.
        """
                ...
                if should_group:
                    ...
                    if self._can_create_group_above(category_a_node.node_id):
                        ...
                        if group_node_id:
                            ...
                            # 그룹 노드 요약 업데이트
                            group_node = self.memory_manager.get_node(group_node_id)
                            await self.update_parent_summary(group_node)
