...
class TreeCleanupEngine:
    ...
    async def _build_similarity_matrix(self, nodes):
        ...
        if self.debug:
            print(f">>>> 유사도 매트릭스 구축: {n}개 노드, {total_pairs}개 쌍")
        
        queries, pairs = [], []
        for i in range(n):
            for j in range(i + 1, n):
                queries.append(f'노드1: "{nodes[i].topic}"\n노드2: "{nodes[j].topic}"\n두 노드가 주제가 유사한가요?')
                pairs.append((i, j))
        
        system_prompt = '두 노드의 주제가 유사하면 "True", 아니면 "False"만 답하라.'
        ...
        for i in range(0, len(queries), BATCH_SIZE):
            ...
            if self.debug:
                print(f">>>> AI 병렬 유사도 판단: {i+1}~{min(i + BATCH_SIZE, total_pairs)} / {total_pairs} 쌍")
            results = await self.ai_manager.call_ai_async_multiple(batch_queries, system_prompt, fine=NODE_SIMILARITY_FINE)
            if self.debug:
                print(f">>>>>>> 배치 소요시간: {time.time() - batch_start:.2f}초")
            for idx, result in enumerate(results):
                if result and result.strip().lower() == 'true':
                    row, col = batch_pairs[idx]
                    matrix[row][col] = matrix[col][row] = True
        
        if self.debug:
            true_count = sum(row.count(True) for row in matrix) // 2
            print(f">>>> 유사도 판단 완료: {true_count}/{total_pairs}개 유사 쌍 발견")
    ...
    def _connected_components_clustering(self, nodes, similarity_matrix):
        ...
        for i in range(n):
            if not visited[i]:
                ...
                while q:
                    u = q.pop(0)
                    cluster.append(nodes[u])
                    for v in range(n):
                        if not visited[v] and similarity_matrix[u][v]:
                            visited[v] = True
                            q.append(v)
    ...
    async def _create_cluster_group(self, parent_id, cluster_nodes):
        ...
        group_node = self.memory_manager.create_category_node(group_topic, group_summary, parent_id)
    ...
    async def _generate_group_name(self, topics):
        ...
        system_prompt = '주어진 주제들을 포괄하는 2~8자의 간결한 한국어 그룹명을 생성하라. 답변은 반드시 따옴표 안에 하나의 명사 단어만 포함해야 한다. 예: "음식"'
        
        try:
            result = await self.ai_manager.call_ai_async_single(f"주제들: {', '.join(topics[:5])}", system_prompt)
            name = self._extract_group_name(result, topics)
            # 빈 문자열, "주제", "이름" 등 무의미한 결과는 무시
            if not name or name.strip() in ["", "주제", "이름"]:
                if self.debug:
                    print(f">>>> [WARN] AI 그룹명 무의미: '{name}' -> fallback 사용")
                return f"{topics[0]} 그룹" if topics else "새 그룹"
            return name
        except Exception as e:
            if self.debug: print(f">>>> [ERROR] 그룹명 생성 오류: {e}")
            return f"{topics[0]} 그룹" if topics else "새 그룹"

    def _extract_group_name(self, ai_response, original_topics):
        """AI 응답에서 실제 그룹명만 추출합니다."""
        if ai_response:
            match = re.search(r'["\"]([^"\\]{2,8})["\"]', ai_response)
            if match:
                return match.group(1).strip()
        return f"{original_topics[0]} 그룹" if original_topics else "새 그룹"

    async def _generate_group_summary(self, nodes):
        """그룹 노드의 요약을 생성합니다."""
        system_prompt = "주어진 노드들의 공통 주제와 내용을 1-2문장으로 간결하게 요약하라."
        nodes_text = "\n".join([f"- {node.topic}:[ {node.summary} ]" for node in nodes])
        try:
            return await self.ai_manager.call_ai_async_single(nodes_text, system_prompt)
        except Exception as e:
            if self.debug: print(f">>>> [ERROR] 그룹 요약 생성 오류: {e}")
