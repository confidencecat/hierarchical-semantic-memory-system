...
async def on_ready():
    ...
    print(f'📊 {len(bot.guilds)}개 서버에 연결되어 있습니다.')
...
async def tree_command(ctx):
    ...
    try:
        ...
        if len(tree_summary) > 1900:
            ...
            # 파일 전송
            await ctx.send(
                f"🌳 **기억 트리가 너무 커서 파일로 전송합니다**\n총 노드 수: {status['total_nodes']}개",
            ...
            embed.add_field(name="📊 통계", value=f"총 노드 수: {status['total_nodes']}개", inline=True)
            await ctx.send(embed=embed)
            
    except Exception as e:
        print(f"❌ 트리 명령어 처리 중 오류: {e}")
    ...
    try:
        ...
        try:
            ...
            if str(reaction.emoji) == "✅":
                ...
                await ctx.send("✅ 서버의 기억이 초기화되었습니다.")
            else:
...
def run_bot():
    ...
    try:
        print("🚀 Discord 봇을 시작하는 중...")
        bot.run(discord_token)
    except discord.LoginFailure:
        print("❌ Discord 토큰이 올바르지 않습니다.")
    except Exception as e:
